...
- Post-fix verification (overflow-safe duration unit conversion): SUCCESS
  ...
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (human-readable size rejects partial numeric parses): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (case-insensitive legacy top_messages stats syntax): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (ISO-8601 strict calendar-date validation): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (day-range strict calendar-date validation): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (parse_utc strict calendar-date validation): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (parse_utc full-input validation): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (absolute-time full-input validation): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (DST-aware local mktime parsing): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (finite-only float condition parsing/evaluation): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (strict whitespace handling for numeric filter values): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (statistics target_field normalization and alias support): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (strict integer validation parity with runtime parsing): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (trimmed stats target_field and blank custom key rejection): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (FilterOperator toString complete enum coverage for case-insensitive/set operators): SUCCESS
  ...
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (time parsing API parity, strict numeric parsing, and Unix timestamp support): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (strict SemVer validation and overflow-safe prerelease comparison): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (field-mapping parser trims whitespace around index/field/format): SUCCESS
  ...
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (legacy numeric value_type supports LOG_LEVEL enum value): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (boolean value validation parity with runtime literals: t/f/yes/no): SUCCESS
  ...
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (SemVer ordering no longer treats build metadata as precedence): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (human-readable size parser rejects malformed whitespace/exponent forms): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (FilterValueType UNKNOWN string aliases round-trip correctly): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (relative-time keywords/parsing are case-insensitive): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (query parser supports additional operator token aliases and unary forms): SUCCESS
  ...
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (field aliases source/line/thread/time map to standard log fields): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
- Post-fix verification (query parser supports EQ/NE/GT/LT/GTE/LTE shorthand operators): SUCCESS
  Commands:
  - cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
  - cmake --build build --parallel
  - ctest --test-dir build --output-on-failure
...
- Passed: 50
- Failed: 0
- Total wall time: 0.78 sec
- Slowest observed tests:
  1) analyzer_Core: 0.49 sec
  2) config_CLI_Filtering: 0.02 sec
  3) config_CLI_Formatting: 0.03 sec
  4) analyzer_LogWriter: 0.02 sec
  5) config_CLI_InputOutput: 0.01 sec
```
