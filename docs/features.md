...
| **Statistical Analysis**     | Generate statistics on log data: entry rates, top messages, log level counts, unique value counts, time-bucket histograms (configurable window), P50/P95/P99 percentiles for any numeric custom field, moving average rate (mean/min/max per bucket), and gap detection (find time gaps between adjacent entries exceeding a threshold). Statistics are available in both batch and `--stream` mode. |
