...
| **Time-based Filtering**     | Filter by absolute time range, relative time (`5m ago`), or for a specific day (`yesterday`, `2023-10-20`). Use `--since DURATION` as a shorthand for the last N (e.g., `--since 1h`). Use `--duration` to define a time window relative to `--start` or `--end`. |
...
| **Statistical Analysis**     | Generate statistics on log data: entry rates, top messages, log level counts, unique value counts, time-bucket histograms (configurable window), P50/P95/P99 percentiles (or a custom percentile set via `percentiles=50;90;99.9`) for standard numeric fields (e.g., `lineNumber`, `id`) or any numeric custom field, moving average rate (mean/min/max per bucket), and gap detection (find time gaps between adjacent entries exceeding a threshold). Statistics are available in both batch and `--stream` mode. In `--stream` mode, `--stats-interval N` emits an intermediate report to stderr every N matching entries, enabling low-latency monitoring of long-running streams. |
| **Deduplication**            | `--dedup-field FIELD` keeps only the first entry per unique value of the named field, removing duplicate log lines. Supports all standard fields (`level`, `message`, `source`, `timestamp` (alias: `time`), `lineNumber`, `threadId`, `module`, `host`, `id`) and any custom field key. |
