...
void printHelp() {
    ...
    std::cout << "  --format <text|json|csv>  Output format (default: text)" << std::endl;
    ...
    std::cout << "  --stream                  Enable streaming mode for large files (incompatible with sorting)" << std::endl;
    std::cout << "  --map-level <FROM=TO>     Map a custom log level string to a standard one (e.g., FATAL=ERROR)" << std::endl;
    std::cout << "  --stats-window <SECONDS>  Show log frequency distribution over a time window" << std::endl;
    std::cout << "  --find-gaps <MS>          Find time gaps in logs longer than the specified milliseconds" << std::endl;
    std::cout << "  --rate                    Calculate and show the average log entry rate (entries/second)" << std::endl;
...
int main(int argc, char* argv[]) {
        ...
        if (arg == "--help") {
            ...
            if (++i < argc) {
                ...
                if (format == "text" || format == "json" || format == "csv") {
                    ...
                    std::cerr << "Error: Invalid output format '" << argv[i] << "'. Must be 'text', 'json', or 'csv'." << std::endl;
                    return 1;
                }
            ...
            if (++i < argc) {
                ...
                if (eqPos != std::string::npos) {
                    ...
                    if (mappedLevel != LogLevel::UNKNOWN) {
                        ...
                        std::cerr << "Error: Invalid target log level for --map-level: '" << to << "'." << std::endl;
                        return 1;
                    }
                } else {
                    std::cerr << "Error: --map-level argument format should be KEY=LEVEL." << std::endl;
                    return 1;
                }
            } else { std::cerr << "Error: --map-level requires an argument." << std::endl; return 1; }
        } else if (arg == "--stats-window") {
            if (++i < argc) {
                try {
                    config.statsWindow = std::chrono::seconds(std::stoi(argv[i]));
                } catch (const std::exception& e) {
                    std::cerr << "Error: Invalid number for --stats-window. " << e.what() << std::endl;
                    return 1;
                }
            } else { std::cerr << "Error: --stats-window requires an argument." << std::endl; return 1; }
        } else if (arg == "--find-gaps") {
            if (++i < argc) {
                try {
                    config.findGapsDuration = std::chrono::milliseconds(std::stoi(argv[i]));
                } catch (const std::exception& e) {
                    std::cerr << "Error: Invalid number for --find-gaps. " << e.what() << std::endl;
                    return 1;
                }
            } else { std::cerr << "Error: --find-gaps requires an argument." << std::endl; return 1; }
        } else if (arg == "--rate") {
    ...
    if (config.streamMode) {
        if (config.sortBy != SortBy::TIMESTAMP || config.sortOrder != SortOrder::ASCENDING) {
            std::cerr << "Error: --stream is incompatible with --sort-by. Streaming mode does not support sorting." << std::endl;
            return 1;
        }
        if (config.showUniqueMessages) {
            std::cerr << "Error: --stream is incompatible with --unique-messages. Unique message counts require global knowledge." << std::endl;
            return 1;
        }
        if (config.showTopMessages) {
            std::cerr << "Error: --stream is incompatible with --top-messages. Top messages require global knowledge." << std::endl;
            return 1;
        }
        if (!config.filePaths.empty() && config.filePaths.size() > 1) {
            std::cerr << "Error: --stream is not compatible with multiple input files. Please provide a single file for streaming." << std::endl;
            return 1;
        }
    }

    if (config.filePaths.empty() && !config.streamMode) { // If no file paths and not in streaming mode
        std::cerr << "Error: No log file path provided and not in streaming mode." << std::endl;
        printHelp();
        return 1;
    }
    ...
    if (!config.streamMode) {
        ...
        if (!config.filePaths.empty()) {
            auto report = analyzer.analyze(config.filePaths[0], config.customPattern); // The analyze method now returns AnalysisReport
            if (report.status != ParseError::SUCCESS && report.status != ParseError::PARTIAL_FAILURE) {
                std::cerr << "Error analyzing file " << config.filePaths[0] << ". Status: ";
            ...
            if (!report.parseErrors.empty()) {
                std::cerr << "Warning: " << report.parseErrors.size() << " lines failed to parse in " << config.filePaths[0] << std::endl;
            }

            for (size_t i = 1; i < config.filePaths.size(); ++i) {
                auto result = analyzer.append(config.filePaths[i], config.customPattern);
                if (!result) { // std::expected holds the error on failure
                    std::cerr << "Error appending file " << config.filePaths[i] << ": " << result.error().message << std::endl;
                    return 1;
                }
                auto appendReport = analyzer.getAnalysisReport(); // Get report for append operation
                 if (!appendReport.parseErrors.empty()) {
                    std::cerr << "Warning: " << appendReport.parseErrors.size() << " lines failed to parse in " << config.filePaths[i] << std::endl;
                }
            }
        }
    }
    ...
    if (config.streamMode) {
        ...
        auto streamEntryCallback = [&](const LogEntry& entry) {
            ...
            if (!criteria.keyword.empty()) {
                if (criteria.keywordCaseSensitive) {
                    matchesKeyword = (entry.message.find(criteria.keyword) != std::string::npos);
                } else {
                    ...
                    matchesKeyword = (messageLower.find(keywordLower) != std::string::npos);
                }
            }
            ...
            if (criteria.startTime && entry.timestamp < *criteria.startTime) matchesTime = false;
            if (criteria.endTime && entry.timestamp > *criteria.endTime) matchesTime = false;
            ...
            if (matchesLevel && matchesKeyword && matchesRegex && matchesTime) {
                if (config.outputFormat == "text") {
                    ...
                    *outputStream << "\"" << analyzer.formatTimestamp(entry.timestamp) << "\",\""
        ...
        if (!config.filePaths.empty()) {
            analyzer.analyzeStream(config.filePaths[0], streamEntryCallback, config.customPattern);
        } else {
            // Should not happen if argument parsing is correct (file path required if not streaming)
            std::cerr << "Error: No file specified for streaming analysis." << std::endl;
            return 1;
        }

    } else { // Not streaming mode
        if (config.statsWindow) {
            *outputStream << "--- Log Frequency Distribution (Window: " << config.statsWindow->count() << "s) ---" << std::endl;
            for (const auto& stats : analyzer.getFrequencyDistributionOptimized(*config.statsWindow)) {
                *outputStream << analyzer.formatTimestamp(stats.windowStart) << ": Total=" << stats.totalCount;
                for (const auto& levelCount : stats.counts) {
                    *outputStream << ", " << LogAnalyzer::logLevelToString(levelCount.first) << "=" << levelCount.second;
                }
                *outputStream << std::endl;
            }
        }
        if (config.findGapsDuration) {
            *outputStream << "--- Time Gaps (Min Duration: " << config.findGapsDuration->count() << "ms) ---" << std::endl;
            for (const auto& gap : analyzer.findTimeGaps(*config.findGapsDuration)) {
                *outputStream << "Gap from " << analyzer.formatTimestamp(gap.start) << " to "
                              << analyzer.formatTimestamp(gap.end) << " (Duration: "
                              << std::chrono::duration_cast<std::chrono::milliseconds>(gap.duration).count()
                              << "ms)" << std::endl;
            }
        }
        if (config.showEntryRate) {
            *outputStream << "--- Average Entry Rate ---" << std::endl;
            *outputStream << "Average entries/second: " << std::fixed << std::setprecision(2) << analyzer.getAverageEntryRate() << std::endl;
        }
        
        // If any specific statistical analysis was requested, we don't proceed with other output types
        // unless they are explicitly requested and make sense.
        // For simplicity, if stats are requested, only stats are printed.
        bool statsRequested = config.statsWindow.has_value() || config.findGapsDuration.has_value() || config.showEntryRate;
