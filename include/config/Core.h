...
inline std::vector<std::string> LogAnalyzerSettings::validate() const {
    ...
    for (const auto& sc : statisticConfigs) {
        ...
        bool has_custom_field_key = (it_custom_field_key != sc.params.end());

        if (sc.type == StatisticType::TOP_MESSAGES || sc.type == StatisticType::TOP_N_FIELD_VALUES) {
            std::string typeStr = (sc.type == StatisticType::TOP_MESSAGES) ? "TOP_MESSAGES" : "TOP_N_FIELD_VALUES";
            if (!has_top_n) {
                errors.push_back("Statistic '" + typeStr + "' requires a 'top_n' parameter.");
            } else {
                try {
                    int topN = std::stoi(it_top_n->second);
                    if (topN <= 0) {
        ...
        if (sc.type == StatisticType::FIELD_VALUE_COUNT || sc.type == StatisticType::TOP_N_FIELD_VALUES) {
            std::string typeStr = (sc.type == StatisticType::FIELD_VALUE_COUNT) ? "FIELD_VALUE_COUNT" : "TOP_N_FIELD_VALUES";
            if (!has_target_field) {
                errors.push_back("Statistic '" + typeStr + "' requires a 'target_field' parameter.");
            } else if (it_target_field->second == "customFields" && !has_custom_field_key) {
