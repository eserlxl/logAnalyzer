...
TEST_F(LogAnalyzerConfigTest, AddDuplicateFieldMapping) {
    ...
    // Removed redundant clearFieldMappings() call as per audit
    settings
...
TEST_F(LogAnalyzerConfigTest, InvalidFieldMappingInput) {
    ...
    // Test with an extremely large group index, assuming it should be rejected or capped.
    // This tests for out-of-bounds issues if the API expects specific ranges.
    settings.addFieldMapping(LogEntryField::MESSAGE, EXTREMELY_LARGE_GROUP_INDEX);
    ASSERT_TRUE(settings.fieldMappings.empty()); // Assuming it's ignored or rejected
}
...
// Test for malformed regex pattern in constructor
TEST_F(LogAnalyzerConfigTest, MalformedRegexPattern) {
    // A malformed regex pattern (unclosed group)
    std::string malformedPattern = R"(^(\d{2}-\d{2}-\d{4} (.*)$)";

    // Test scenario: Constructor throws or sets a default/error state
    try {
        LogAnalyzerSettings malformedSettings(malformedPattern);
        ASSERT_NE(malformedSettings.lineParsePattern, malformedPattern);
        ASSERT_EQ(malformedSettings.lineParsePattern, std::string(DEFAULT_LOG_REGEX_PATTERN_INTERNAL));
    } catch (const std::regex_error& e) {
        SUCCEED() << "Caught expected std::regex_error for malformed pattern: " << e.what();
    } catch (...) {
        FAIL() << "Caught an unexpected exception for malformed pattern.";
    }
}
