...
void createDummyLogFile(const std::string& filename, const std::vector<std::string>& lines) {
...
TEST_F(LogAnalyzerTest, GetFrequencyDistribution) {
    ...
    auto distribution = analyzer.getFrequencyDistribution(std::chrono::seconds(10));
...
TEST_F(LogAnalyzerTest, MergeAnalyzers) {
    createDummyLogFile("log1.log", {
        "[2023-01-01 10:00:00] INFO: From log1",
        "[2023-01-01 10:00:02] WARNING: From log1"
    });
    createDummyLogFile("log2.log", {
        "[2023-01-01 10:00:01] ERROR: From log2",
...
TEST_F(LogAnalyzerTest, PrintFilteredEntriesCustomFormat) {
    ...
    analyzer.printFilteredEntries(oss, criteria, "Level: {level}, Time: {timestamp}, Msg: {message}");
